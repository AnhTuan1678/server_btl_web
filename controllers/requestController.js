...
const updateStatus = async (req, res) => {
  ...
  const query = 'UPDATE relief_areas SET status = ?, updated_at = NOW() WHERE id = ?';
...
const getItemUsedByRequest = async (req, res) => {
  ...
  try {
    ...
    const [items] = await db.query('SELECT * FROM items WHERE is_used = 1 AND area_id = ?', [
      requestId,
    ]);
...
const AddArticle = async (req, res) => {
  ...
  try {
    const query = `
      ...
      VALUES (?, ?, ?)
    `;
    await db.execute(query, [name, url, requestId]);
